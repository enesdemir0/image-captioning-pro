...
# Encoder output grid sizes (H*W spatial features per backbone)
...
def main():
    ...
    # ── 3. Dual Checkpoint Paths (Drive + Local Session Cache) ───────────────
    ...
    tokenizer_path_drive = os.path.join(drive_ckpt_dir, f"{model_id}_tokenizer.json")
    tokenizer_path_local = os.path.join(local_ckpt_dir, f"{model_id}_tokenizer.json")
    ...
    if os.path.exists(enc_path_drive) and os.path.exists(meta_path):
        ...
        if start_epoch < epochs:
            ...
            encoder.load_weights(enc_path_drive)
            decoder.load_weights(dec_path_drive)
        else:
            if not os.path.exists(tokenizer_path_drive):
                loader.text_processor.save_tokenizer(tokenizer_path_drive)
            if not os.path.exists(tokenizer_path_local):
                loader.text_processor.save_tokenizer(tokenizer_path_local)
    ...
    with mlflow.start_run(run_name="Training_Session"):
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for batch, (img, tgt) in enumerate(train_ds.take(steps_per_epoch)):
                ...
                if batch % 5 == 0:
                    print(f"E{epoch+1} B{batch}/{steps_per_epoch} Loss {t_l/(batch+1):.4f}")

            v_l = 0
            for _, (v_img, v_tgt) in enumerate(val_ds.take(val_steps)):
            ...
            # Checkpoint every 5 epochs and on the final epoch
            if (epoch + 1) % 5 == 0 or (epoch + 1) == epochs:
                encoder.save_weights(enc_path_drive)
                decoder.save_weights(dec_path_drive)
                encoder.save_weights(enc_path_local)
                decoder.save_weights(dec_path_local)

                # Persist frozen vocabulary to both locations
                loader.text_processor.save_tokenizer(tokenizer_path_drive)
                loader.text_processor.save_tokenizer(tokenizer_path_local)

                with open(meta_path, 'w') as f:
                    json.dump({'last_completed_epoch': epoch + 1}, f)
                print(f"Checkpoint saved (local + Drive).")
