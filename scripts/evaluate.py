...
def plot_attention_grid(image, result, attention_plot, sample_idx, model_id):
    """
    FIXED: Uses the correct extent logic for Matplotlib.
    """
    ...
    for i in range(len(words)):
        ...
        # 1. Plot the base image
        ax.imshow(temp_image)
        
        # 2. THE FIX: Overlay heatmap using the image's coordinate system
        # extent=[left, right, bottom, top]
        ax.imshow(att_map, cmap='gray', alpha=0.5, extent=(0, img_w, img_h, 0), interpolation='bilinear')
        ax.axis('off')
...
def main():
    ...
    for e_file, d_file in zip(possible_enc, possible_dec):
        ...
        if os.path.exists(e_path):
            ...
            print(f"✅ Loaded weights: {e_file}")
            loaded = True; break
    
    if not loaded:
        print(f"❌ Weights not found for {model_id}"); return

    b1_l, b2_l, b3_l, b4_l, met_l, rou_l = [], [], [], [], [], []
    with mlflow.start_run(run_name="Final_Evaluation"):
        os.makedirs("results/samples", exist_ok=True)
        print(f"--- Final Evaluation: {model_id} ---")
        for i in range(min(50, len(test_imgs))):
            ...
            if i < 5:
                ...
                plt.figure(figsize=(10, 10)); plt.imshow(display_img)
        ...
        print(f"B4: {summary['test_bleu4']:.4f} | METEOR: {summary['test_meteor']:.4f}")
